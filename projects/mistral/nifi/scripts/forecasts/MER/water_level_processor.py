...
def parse_resolution_to_meters(text: str) -> float:
    ...
    raise ValueError(f"Unsupported resolution format '{text}'. Use values like 500m or 1km.")


def parse_resolutions(raw_values: Iterable[str]) -> List[Tuple[str, float]]:
...
class WaterLevelProcessor:
    def __init__(
        ...
        map_offset_hours: float = 24.0,
        timeseries_offset_hours: float = 24.0,
        ...
        if self.produce_maps and len(self.resolutions) == 0:
            raise ValueError("Map output selected but no --resolutions were provided.")
        if self.produce_timeseries and self.stations_path is None:
            raise ValueError("Timeseries output selected but stations_path was not provided.")
        ...
        self.stations = load_stations(self.stations_path) if self.stations_path is not None else []

        self.station_offsets: Dict[str, Tuple[float, float]] = {}
        if self.produce_timeseries and self.station_offsets_path is None:
            logger.warning("No station offsets file provided: using zero offsets for all stations.")
        elif self.produce_timeseries and not self.station_offsets_path.exists():
            logger.warning(
                "Station offsets file not found ({}): using zero offsets for all stations.",
                self.station_offsets_path,
            )
        elif self.produce_timeseries:
        ...
        self.map_time_indices = self._time_indices_after_offset(self.map_offset_hours)
        self.timeseries_time_indices = self._time_indices_after_offset(self.timeseries_offset_hours)
        ...
        self.station_matches = self._match_stations_to_nodes() if self.produce_timeseries else []
        self.grid_specs = self._build_grid_specs() if self.produce_maps else {}
        logger.info(
            "Configured run: {} map timesteps after {}h, {} timeseries timesteps after {}h",
        ...
        logger.info(
            "Enabled outputs: maps={}, timeseries={}",
            self.produce_maps,
        ...
        if self.produce_timeseries:
            self._summarize_indices("Time series", self.time_values, self.timeseries_time_indices)
    ...
    def _time_indices_after_offset(self, offset_hours: float) -> np.ndarray:
        ...
        start_time = self.time_values[0] + np.timedelta64(int(round(offset_hours * 3600.0)), "s")
    ...
    @staticmethod
    def _summarize_indices(label: str, time_values: np.ndarray, indices: np.ndarray) -> None:
        if time_values.size == 0:
    ...
    def _build_triangulation(self) -> mtri.Triangulation:
        ...
        return mtri.Triangulation(self.node_lon, self.node_lat, triangles=self.triangles, mask=tri_mask)
    ...
    def _build_grid_specs(self) -> Dict[str, GridSpec]:
        ...
        for label, meters in self.resolutions:
    ...
    def run(self) -> None:
        ...
        for match in self.station_matches:
            m_obs, m_mod = get_station_offset_pair(match.station.station_id, self.station_offsets)
            station_correction = get_station_correction(match.station.station_id, self.station_offsets)
        ...
        if self.produce_maps:
            for label in self.grid_specs:
                (self.output_dir / f"geotiff_{self.source_tag}_{label}").mkdir(parents=True, exist_ok=True)
        ...
        if self.produce_maps:
            for t_idx in self.map_time_indices:
                ...
                values = sanitize_values(
                    np.asarray(water_level_da.isel(time=t_idx).values, dtype=np.float64),
                    self.invalid_values,
                )
                values = values + self.geotiff_field_offset

                z = np.ma.array(values, mask=(~np.isfinite(values) | ~self.static_valid))
                interpolator = mtri.LinearTriInterpolator(self.triangulation, z)

                for label, spec in self.grid_specs.items():
                    grid_values = interpolator(spec.mesh_lon, spec.mesh_lat)
                    grid_data = np.asarray(np.ma.filled(grid_values, np.nan), dtype=np.float32)
                    grid_data[~np.isfinite(grid_data)] = np.nan

                    out_file = (
                        self.output_dir
                        / f"geotiff_{self.source_tag}_{label}"
                        / f"water_level_{self.source_tag}_{label}_{safe_ts}.tif"
                    )
                    self._write_geotiff(out_file, grid_data, spec)
                logger.info("Wrote maps for timestep {}", timestamp)

        if self.produce_timeseries:
            timeseries_indices = set(int(v) for v in self.timeseries_time_indices)
            ...
            if timeseries_indices:
                first_ts = np.datetime_as_string(self.time_values[min(timeseries_indices)], unit="s")
                last_ts = np.datetime_as_string(self.time_values[max(timeseries_indices)], unit="s")
                ts_suffix = f"{first_ts.replace(':', '').replace('-', '')}_{last_ts.replace(':', '').replace('-', '')}"
            else:
                ts_suffix = "empty"

            ts_out = self.output_dir / f"station_timeseries_{self.source_tag}_{ts_suffix}.json"
    ...
    @staticmethod
    def _write_geotiff(path: Path, data: np.ndarray, spec: GridSpec) -> None:
        profile = {
            "driver": "GTiff",
            "height": data.shape[0],
            "width": data.shape[1],
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Optional list like 500m 1km or '[\"500m\",\"1km\"]'. Needed for map output.",
    )
    parser.add_argument("--output-dir", type=Path, required=True, help="Output directory path.")
    parser.add_argument(
        "--invalid-values",
        nargs="*",
        type=float,
...
def main() -> None:
    ...
    resolutions = parse_resolutions(args.resolutions) if args.resolutions else []
