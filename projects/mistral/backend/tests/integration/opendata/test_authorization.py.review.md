...
> Dominio **opendata** marcato `runtime_sensitive`: entrambi i test vengono eseguiti senza skip dipendenti dal catalogo runtime.
...
| `create_private_opendata_env` | helper locale | [opendata/support.py](projects/mistral/backend/tests/integration/opendata/support.py) | Crea dataset **privato** isolato + utente (non autorizzato) + 1 file opendata reale; l'attribution è sintetica e non esistono precondizioni di skip. |
...
- **Setup**: come sopra, ma con l'utente collegato al dataset; le due response di download sono registrate nel cleanup e chiuse prima della cancellazione del file.
- **Assert**:
...
- **Nessuno skip silenzioso**: `create_private_opendata_env` usa il dataset helper condiviso, che crea attribution, license e group license sintetici. La verifica di sicurezza viene sempre eseguita.
...
- **File reale con cleanup verificato**: il secondo test rilegge davvero il contenuto scritto su `/opendata`; entrambe le response streamate vengono chiuse prima di `_delete_file`, che asserisce la scomparsa del path.
...
- [x] Verificato il teardown del file reale, delle request, dell'utente e del bundle dataset nella run completa opendata.
...
|---|---|---|---|---|---|
| `test_private_dataset_endpoints_reject_unauthorized_access` | 3 `get` opendata + `check_dataset_authorization` | `401` per anonimo e loggato-non-autorizzato su listing/download/file | — (DB+FS reali) | `client`, `cleanup_registry`, `create_private_opendata_env` | No |
| `test_private_dataset_endpoints_allow_authorized_user` | 3 `get` opendata (rami autorizzati) | `200` + contenuti corretti dopo aggancio m2m | — (DB+FS reali) | `client`, `cleanup_registry`, `create_private_opendata_env`, `authorize_user_for_dataset` | No |
