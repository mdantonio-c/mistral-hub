...
class TestApp(BaseTests):
    ...
    def get_params_value(client, headers, db_type):
        ...
        for d in obs_dataset:
            ...
            if response_data['items']:
                log.debug('api fields db type {} : response data: {}', db_type, response_data)
                break
    ...
    def standard_observed_endpoint_testing(self, client, headers, q_params):
        ...
        endpoint = API_URI + '/observations?q=reftime:>={date_from},<={date_to}&networks={network}&lat={lat}&lon={lon}&stationDetails=true'.format(
            date_from=q_params['date_from'], date_to=q_params['date_to'],network=q_params['network'], lat=station_lat_example, lon=station_lon_example)
        r = client.get(endpoint, headers=headers)
        # check response code
        assert r.status_code == hcodes.HTTP_OK_BASIC
        ...
        endpoint = API_URI + '/observations?q=reftime:>={date_from},<={date_to}&networks={network}&lat={lat}&lon={lon}&stationDetails=true'.format(
            date_from=q_params['date_from'], date_to=q_params['date_to'],
            network=random_net,lat=station_lat_example, lon=station_lon_example)
        r = client.get(endpoint, headers=headers)
